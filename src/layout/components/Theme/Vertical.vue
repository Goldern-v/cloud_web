...
const sidebarClass = computed(() => {
  const sidebarOpened = store.appStore.sidebarOpened
    ? 'aside-expend'
    : 'aside-compress'
  const isDark =
    store.appStore.theme.sidebarStyle === 'dark' ? 'sidebar-dark' : ''
...
.navbar-container {
  ...
  display: flex;
  align-items: center;
  background: $headerNavbarBgColor; // var(--theme-header-bg-color);
