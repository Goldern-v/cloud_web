  ...
  <div class="network-config">
    <el-form ref="formRef" :model="form" :rules="rules" label-position="left">
      <el-card>
        ...
        <el-form-item label="增强高速网卡">
          <div>
            <div v-for="(item, index) of form.enhanceCard" :key="index" class="flex-row" style="margin-bottom: 10px;">
              <div class="ideal-default-margin-right">增强高速网卡{{ index + 1 }}</div>
              <el-select
                v-model="item.value"
                placeholder="请选择"
                class="ideal-default-margin-right"
              >
                <el-option
                  v-for="item of state.enhanceCardList"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
              ...
              <svg-icon
                v-if="form.enhanceCard.length > 1"
                icon="circle-close"
                class="ideal-default-margin-right"
                @click="clickDeleteEnhanceCard(index)"
              />
            </div>

            <div class="flex-row">
              <el-button link @click="clickAddEnhanceCard">
                <svg-icon icon="circle-add" class="ideal-svg-margin-right"/>
                增加一块增强高速网卡
              </el-button>
              <div class="ideal-tip-text">您还可以增加 1 块增强高速网卡</div>
            </div>
          </div>
        </el-form-item>
      </el-card>
        ...
        <el-form-item label="安全组">
          <div class="flex-column" style="width: 100%">
            ...
            <div class="flex-row">
              ...
              <el-button link type="primary">配置安全组规则</el-button>
            </div>
      ...
      <el-card class="ideal-large-margin-top">
        <el-form-item label="弹性公网IP">
          <div class="flex-column">
            ...
            <div v-if="form.ipMode === '1'" class="ideal-tip-text">
              自动为每台裸金属服务器分配独享带宽的弹性公网IP。
            </div>
            <div v-if="form.ipMode === '2'" class="ideal-tip-text">
              为裸金属服务器分配已有弹性公网IP，不能批量申请裸金属服务器。
            </div>
...
const form = reactive({
  ...
  expand: [] as any[], // 扩展网卡
  enhanceCard: [{ value: '' }], // 增强高速网卡
  safeGroup: [], // 安全组
  safeGroupInfo: '', // 安全组(供确认配置界面使用)
