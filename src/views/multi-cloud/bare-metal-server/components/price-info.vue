    ...
    <div class="flex-row price-footer-box">
      ...
      <div class="flex-row price-footer-box--item">
        <el-button
          ...
          >上一步</el-button
        >
        <el-button v-if="stepsIndex !== 4" type="primary" @click="handleNext"
          >下一步</el-button
        >
        <el-button
          v-if="stepsIndex === 4"
...
// 是否显示包年包月
const showPackage = computed(() => props.stepsIndex !== 4)
// 包年包月时长
...
// 询价
const getInquiry = () => {
  if (!resourcePoolInfo?.value || !props?.basicData?.billingMode || !props?.basicData?.currentSpec || !props?.basicData?.systemDiskSize) { return }
