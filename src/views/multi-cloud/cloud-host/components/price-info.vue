    ...
    <div class="flex-row price-footer-box">
      <div class="flex-row price-footer-box--item">
        ...
        <div class="show-price">
          ¥{{ price }}元{{ !isPackage ? '/小时' : '' }}
        </div>

        <el-select
          v-if="isPackage && showPackage"
          v-model="timeValue"
      ...
      <div class="flex-row price-footer-box--item">
        <el-button
          ...
          >上一步</el-button
        >
        <el-button v-if="stepsIndex !== lastStep" type="primary" @click="handleNext"
          >下一步</el-button
        >
        <el-button
...
watch(() => props.basicData.billingMode, value => {
...
// 是否显示包年包月
const showPackage = computed(() => props.stepsIndex !== 3)
// 包年包月时长
const timeValue = ref(1)
watch(() => timeValue.value, value => {
...
// 询价
const getInquiry = () => {
  if (!resourcePool?.value || !props?.basicData?.billingMode || !props?.basicData?.currentSpec || !props?.basicData?.systemDiskSize) { return }
