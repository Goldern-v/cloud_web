...
import vpcNet from './vpc-net.vue'
import publicNet from './public-net.vue'
...
const labelArray = ref([
  ...
  { label: 'UUID', prop: 'uuid', isCopy: true },
  { label: 'cpunum', prop: 'cpunum' },
  { label: '内存', prop: 'memory' },
  { label: '运行平台', prop: 'system' },
...
// 标签页组件
const tabs: any = { vpcNet, publicNet }
// tabs标签页
const tabControllers = ref([
  { label: 'VPC网络', name: 'vpcNet' },
  { label: '公有网络', name: 'publicNet' }
])
const activeName = ref('vpcNet')

const handleClick = (tab: any) => {
  if (tab.paneName === 'vpcNet') {
    // queryDetail()
  }
}
</script>
