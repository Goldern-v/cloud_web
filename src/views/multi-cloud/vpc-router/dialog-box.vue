...
import loadLayer3Network from './components/load-layer3-network.vue'
import setNicQos from './components/set-nic-Qos.vue'
...
const showLoadLayer3Network = computed(() => props.type === 'loadLayer3Network') //加载三层网络
...
const showSetNicQos = computed(() => props.type === 'setNicQos') //删除
...
const initDialog = () => {
  ...
  } else if (showLoadLayer3Network.value) {
    dialogWidth.value = '40%'
    dialogTitle.value = '加载三层网络'
  ...
  } else if (showSetNicQos.value) {
    dialogWidth.value = '25%'
    dialogTitle.value = '设置网卡Qos'
  }
}
