...
const isRenew = computed(() => props.operateType === 'renew') //续订页面

// 监听询价接口用到的传参数据
watch(
  () => [
    props.basicData.billingMode,
    props.basicData.line,
    props.basicData.eipPurchaseDuration,
    props.basicData.bandwidthSize,
    props.basicData.bandwidthType
  ],
  value => {
    getInquiry()
  },
...
const isPackage = computed(
  () => props.basicData.billingMode === BillingEnum.PACKAGE
) //是否包年包月
...
const renewParams = () => {
  ...
  const item = {
    code: eipType,
    specs: bandwidth.size
  }
  let params: { [key: string]: any } = {
    billCycle: eipPurchaseDuration < 10 ? 'MONTH' : 'YEAR',
    billType,
    cycleNum: eipPurchaseDuration,
    cloudPlatformId,
    itemsList: [item],
    name,
    orderType: 'RENEW',
