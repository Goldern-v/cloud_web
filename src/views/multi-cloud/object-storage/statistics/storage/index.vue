<template>
  <div class="storage">
    ...
    <line-view
      v-for="(item, index) of storageData"
      :key="index"
      ref="chart"
      :item="item"
      :statistics-data="item.statisticsData"
      :statistics-value="item.statisticsValue"
      class="storage-chart ideal-middle-margin-top"
    />
  </div>
</template>
...
onMounted(() => {
  storageData.value = [
    { statisticsValue: [], statisticsData: [], cnName: '存储总趋势', enName: 'test1', max: 100, min: 2 },
    { statisticsValue: [], statisticsData: [], cnName: '存储类型趋势', enName: 'test2', max: 100, min: 2 },
    { statisticsValue: [], statisticsData: [], cnName: '存储各类数量趋势', enName: 'test3', max: 100, min: 2 }
  ]
})
/**
