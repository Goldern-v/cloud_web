      ...
      <template #name>
        <el-table-column label="名称/ID" show-overflow-tooltip>
          <template #default="props">
            <el-button link type="primary" @click="clickDetail">{{ props.row.name }}</el-button>
...
state.dataList = [
  { name: 'vault-5a27', uuid: '98a2e32b-09a1-0321-0acd', type: '备份', status: '可用', statusType: 'status-success', spec: '--', storedSize: 5, allSize: 30, backupPolicy: '启用', backupPolicyStr: 'defaultPolicy', bound: 1, billingMode: '按需计费' }
]
...
const tableHeaders: IdealTableColumnHeaders[] = [
  ...
  { label: '已绑定服务器(个)', prop: 'bound' },
  { label: '付费方式', prop: 'billingMode' }
]
// 列表行数据操作事件
const operateBtns: IdealTableColumnOperate[] = [
  { title: '绑定服务器', prop: 'bindEcs' },
...
const clickOperateEvent = (command: string | number | object, row: any) => {
  if (command === 'bindEcs') {
    ...
    dialogType.value = 'bindEcs'
  } else if (command === 'execute') {
    router.push({ path: '/multi-cloud/cloud-host-backup-storage/execute-backup' })
  }
}
// 列表左侧按钮
const leftButtons = ref<IdealButtonEventProp[]>([
  ...
  { title: '删除', prop: 'delete', disabled: true, disabledText: '请选择存储库' },
  { title: '绑定备份策略', prop: 'bind', disabled: true, disabledText: '请选择存储库' }
])
const clickLeftEvent = (value: string | number | object) => {
  if (value === 'bind') {
    ...
    router.push({ path: '/multi-cloud/cloud-host-backup-storage/create'})
  }
}
// 搜索
...
const clickDetail = () => {
  router.push({ path: '/multi-cloud/cloud-host-backup-storage/detail' })
}
</script>
