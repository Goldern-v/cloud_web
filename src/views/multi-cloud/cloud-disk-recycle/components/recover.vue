...
import { cloudDiskRecycleRecover } from '@/api/java/store'
...
const submitForm = () => {
  ...
  cloudDiskRecycleRecover(params)
    .then((res: any) => {
      const { code, data } = res
      if (code === 200) {
        ElMessage.success('云硬盘恢复成功')
        emit(EventEnum.success)
      } else {
        ElMessage.error(data.msg || '云硬盘恢复失败')
      }
      hideLoading()
    })
    .catch(_ => {
