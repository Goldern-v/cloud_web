<template>
  <div class="ideal-main-container cloud-disk-recycle">
    <ideal-select-search
    ...
    >
      <template #name>
        <el-table-column label="名称/ID" show-overflow-tooltip>
          <template #default="props">
            <div class="cloud-disk-recycle-title" @click="clickDetail(props.row)">
              {{ props.row.name }}
            </div>
      ...
      <template #pay>
        <el-table-column label="付费/创建时间" show-overflow-tooltip>
          <template #default="props">
...
const clickOperateEvent = (command: leftBtnType, row: any) => {
  rowData.value = row
  if (command === 'recover') {
