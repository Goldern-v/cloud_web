          ...
          <template #default="props">
            <div class="ideal-theme-text">
              <span
                ...
                @mouseenter="isShowCopy = true"
                @mouseleave="isShowCopy = false"
              ...
              <svg-icon
                ...
                icon="copy-icon"
                class-name="copy-svg"
                @click="clickCopy(props.row.name)"
              />
            </div>
          </template>
        </el-table-column>
...
const operateButtons: IdealTableColumnOperate[] = [
  ...
  { type: 'primary', title: '添加网络段', prop: 'addNetSegment' },
...
const clickOperateEvent = (command: string | number | object, row: any) => {
  ...
  if (command === 'edit') {
    ...
    handleDelete(row)
  } else if (command === 'addNetSegment') {
    dialogType.value = 'addNetSegment'
  }
}

const handleDelete = (row: any) => {
  ElMessageBox.confirm('确定要删除当前管理网络吗？', '删除', {
...
const clickRefreshEvent = () => {
  ...
  if (dialogType.value === 'resourcePool') {
    router.push({
      path: '/multi-cloud/manage-network/create'
    })
  } else {
    getDataList()
  }
