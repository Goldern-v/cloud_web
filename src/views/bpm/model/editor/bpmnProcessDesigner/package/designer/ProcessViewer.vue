...
import {
  DICT_TYPE,
  getIntDictOptions
} from '@/views/bpm/model/editor/utils/dict'

import { formatDate } from '@/views/bpm/model/editor/utils/formatTime'
  ...
  bpmnModeler.value
    ...
    .rootElements[0].flowElements?.forEach((n: any) => {
      ...
      if (n.$type === 'bpmn:UserTask') {
        let activityArr: any = activityList.filter((m: any) => m.key === n.id)
        let activityIds = activityArr.map(
          (v: { [index: string]: string | number }) => v.taskId
        )
        let taskArr = taskList.value.filter(
          (v: { [index: string]: string | number }) =>
            activityIds.includes(v.id)
        )
        if (!taskArr.length) {
          return
        }
        const task = taskArr.find(v => v.result !== 4)
        // let result1 = taskArr.find(v => v.result == 1)
        // if(result1){
        //   return canvas.addMarker(n.id, getResultCss(result1.result))
        // }

        // 用户任务
        // 处理用户任务的高亮
        // const task: any = taskList.value.find(
        //   (m: any) => m.id === activity.taskId
        // ) // 找到活动对应的 taskId
        if (!task) {
          return
        }
        ...
        outgoing?.forEach((nn: any) => {
          // debugger
          let targetActivity: any = activityList.find(
            (m: any) => m.key === nn.targetRef.id
          )
...
const initModelListeners = () => {
  ...
  EventBus.on('element.out', function (eventObj: any) {
    let element = eventObj ? eventObj.element : null
    elementOut(element)
  })
}
...
const elementHover = (element: any) => {
  ...
  let activityArr = activityLists.value.filter(m => m.key === element.value.id)
  let activityIds = activityArr.map(v => v.taskId)
  ...
  if (
    ...
    element.value.type !== 'bpmn:Process'
  ) {
    ...
    if (element.value.type === 'bpmn:StartEvent' && processInstance.value) {
      html = `<p>发起人：${processInstance.value.startUser.username}</p>
                  <p>创建时间：${formatDate(processInstance.value.createTime)}`
    } else if (element.value.type === 'bpmn:UserTask') {
      // debugger

      let task = taskList.value.find(m => activityIds.includes(m.id))

      console.log('hover0------------hover--hover', taskList.value, activity)
      if (!task) {
        return
      }
      ...
      html = `<p class="model-hover">审批人：${task.assigneeUser.username}</p>
                  <p>VDC：${task.assigneeUser.vdcCode || '--'}</p>
                  <p>结果：${dataResult}</p>
      ...
      element.value.type === 'bpmn:ServiceTask' &&
...
watch(
  ...
  newProcessInstanceData => {
    ...
    createNewDiagram(xml.value)
    console.log('-----watch-instance---', processInstance.value)
  }
)
watch(
  () => props.taskData,
