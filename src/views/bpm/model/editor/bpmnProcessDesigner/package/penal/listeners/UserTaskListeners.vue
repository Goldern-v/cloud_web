  ...
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="small" border>
      ...
      <el-table-column
        ...
        :formatter="(row:any) => (listenerEventTypeObject as any)[row.event]"
      />
      <el-table-column
        label="事件id"
        min-width="80px"
...
type listenerEventTypeObject = {
  [key: string]: string
}
type objectType = {
  [key: string | number]: any
}
const listenerEventTypeObject = ref<listenerEventTypeObject>(eventType)
const listenerTypeObject = ref<objectType>(listenerType)
const listenerFormModelVisible = ref(false)
const listenerForm = ref<any>({})
const fieldTypeObject = ref<objectType>(fieldType)
...
const resetListenersList = () => {
  ...
  bpmnElementListeners.value =
    bpmnElement.value.businessObject?.extensionElements?.values.filter(
      (ex: { $type: string }) => ex.$type === `${prefix}:TaskListener`
    ) ?? []
