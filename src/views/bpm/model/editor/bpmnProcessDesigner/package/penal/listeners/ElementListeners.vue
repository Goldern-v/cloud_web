...
type listenerTypeObject = {
  [key: string]: string
}
const listenerTypeObject = ref<listenerTypeObject>(listenerType)
const fieldTypeObject = ref<{ [index: string | number]: any }>(fieldType)
...
const resetListenersList = () => {
  ...
  bpmnElementListeners.value =
    bpmnElement.value.businessObject?.extensionElements?.values?.filter(
      (ex: { [index: string]: any }) =>
        ex.$type === `${prefix}:ExecutionListener`
    ) ?? []
