...
const tableHeaders: IdealTableColumnHeaders[] = [
  { label: '业务组', prop: 'name' },
  { label: '预算', prop: 'budgetText' },
  { label: '重置周期', prop: 'resetCycle' },
  { label: '使用率', prop: 'usageRate', useSlot: true },
  { label: '已使用', prop: 'useText' },
  { label: '剩余', prop: 'remainderText' }
]
const cycleFormat: {[key: string]: any} = {
  FOREVER: '无',
  WEAK: '周',
  MONTH: '月',
  YEAR: '年'
...
watch(
  ...
  arr => {
    arr?.forEach(item => {
      ...
      item.budgetText = item.budget + '¥'
      item.useText = item.use + '¥'
      item.remainderText = item.remainder + '¥'
    })
  }
)
const cancelForm = () => {
