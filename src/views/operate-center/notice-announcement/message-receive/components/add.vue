...
import { receiverManageList, messageReceiveConfigAdd, messageReceiveConfigAddBatch } from '@/api/java/operate-center'
...
const getPlatformUser = () => {
  getPlatformUserList().then((res: any) => {
    const { code, data } = res
    if (code === 200) {
...
const getReceiverList = () => {
  receiverManageList().then((res: any) => {
    const { code, data } = res
    if (code === 200) {
      receivers.value = data
    } else {
