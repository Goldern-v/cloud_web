...
const clickOperateEvent = (command: string | number | object, row: any) => {
  ...
  if (command === 'detail') {
    router.push({
      path: '/operate-center/notice-announcement/announcement-manage/publicity/detail',
      query: { id: row?.id }
    })
...
const tableRef = ref()
const handleBatch = () => {
  ElMessageBox.confirm('确认下架这些通知公告？', '下架公告', {
    confirmButtonText: '确认',
    cancelButtonText: '取消'
  }).then(() => {
    const arr = state.dataListSelections?.map((item: any) => item?.id)
    announcementBatchOut(arr).then((res: any) => {
      const { code } = res
      if (code === 200) {
        ElMessage.success('下架成功')
