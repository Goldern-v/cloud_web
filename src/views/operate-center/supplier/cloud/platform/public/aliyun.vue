<template>
  <div class="aliyun">
    <div class="aliyun__container">
      <div class="flex-row aliyun__tip">
        <svg-icon
          icon="info-warning"
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-position="left">
        ...
        <el-form-item label="站点" prop="site">
          <el-select v-model="form.site" placeholder="请选择" style="width: 20%;">
            <el-option
              v-for="(item, idx) of siteList"
              :key="idx"
              :label="item.label"
              :value="item.value"
            >
...
const props = withDefaults(defineProps<AliyunProps>(), {
  cloudCategory: '',
  cloudType: ''
})
...
const handleCreate = () => {
  ...
  cloudPlatformCreate(params).then((res: any) => {
    ...
    if (code === 200) {
      ...
      router.push({
        path: '/operate-center/supplier/cloud/platform/list'
      })
    } else {
