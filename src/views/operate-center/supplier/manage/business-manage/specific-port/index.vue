...
const onClickSearch = (v: IdealTextProp[]) => {
  state.queryForm = {}
  state.queryForm.searchSpecializedPort = true
  if (v.length) {
  ...
  val => {
    if (val?.length) {
      val.forEach((item: any) => {
        ...
        item.deliveryPeriod = item.deliveryDuration
          ? `${item.deliveryDuration}天`
          : ''
        item.nrcStr = item.nrc ? `${item.nrc}$` : ''
        item.mrcStr = item.mrc ? `${item.mrc}$` : ''
      })
    }
  }
)
...
const clickLeftEvent = (command: string | number | object) => {
  if (command === 'dataEntry') {
    ...
    dialogType.value = 'specificPortDataEntry'
  }
}
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确定要删除当前端口信息吗？', '删除', {
    ...
    .then(() => {
      cloudDataDelete({ id: row.id }).then((res: any) => {
        ...
        if (code === 200) {
          getDataList()
          ElMessage.success('删除端口信息成功')
        } else {
          ElMessage.error('删除端口信息失败')
        }
      })
    })
    .catch(() => {
      ElMessage.info('取消端口信息')
    })
}
