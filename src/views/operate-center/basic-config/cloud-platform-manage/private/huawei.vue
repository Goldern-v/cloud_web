  ...
  <div class="huawei-cloud">
    <div class="huawei-cloud__container">
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-position="left">
        ...
        <el-form-item label="租户ID" prop="cloudAccount">
          <el-input
            v-model="form.cloudAccount"
            :disabled="isEdit"
            class="custom-input"
          ></el-input>
        </el-form-item>

        <el-form-item label="云用户ID" prop="cloudUser">
          <el-input
            v-model="form.cloudUser"
            :disabled="isEdit"
            class="custom-input"
          ></el-input>
        </el-form-item>

        <el-form-item label="域名" prop="billPrice">
          <div class="flex-column" style="width: 60%">
            ...
            <ideal-table-list
              :table-data="form.domainAddresses"
              ...
              <template #operation>
                <el-table-column label="操作" width="60">
                  <template #default="props">
                    <svg-icon
                      icon="delete-icon"
                      @click="deleteDomain(props.$index)"
                    ></svg-icon>
                  </template>
                </el-table-column>
              </template>
...
const initEditData = () => {
  cloudPlatformDetail({ id }).then((res: any) => {
    ...
    if (code === 200) {
      ...
      form.cloudAccount = data?.secret?.cloudAccount
      form.cloudUser = data?.secret?.cloudUser
      form.mode = !!data?.mode
      form.enableMonitor = !!data?.enableMonitor
      form.billConfig = data?.costConfig
      form.billSyncConfig = data?.costSyncConfig
      form.domainAddresses = data?.domainAddresses
      form.cloudVersion = data?.cloudVersion
...
const handleCreate = () => {
  const params = {
    ...
    accessCredentialType: 2, // 云底层访问类型 0：账号密码、1：密钥 2：都包含
    password: {
    ...
    domainAddresses: form.domainAddresses, // 域名地址列表
    cloudVersion: form.cloudVersion // 云版本
  }
...
const deleteDomain = (index: number) => {
  form.domainAddresses.splice(index, 1)
}
...
const clickRefreshEvent = (value: any) => {
  ...
  if (dialogType.value === 'addDomain') {
    form.domainAddresses.push(value)
  }
}
</script>
