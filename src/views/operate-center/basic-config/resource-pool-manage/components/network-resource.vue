  ...
  <div class="storage-resource">
    <div class="storage-resource-container">
      <div class="flex-row info-form__tip ideal-middle-margin-bottom">
        ...
        ></svg-icon>
        <span
          >您可以设置该资源池的网络资源配额，控制用户可以申请的资源数量。</span
        >
      </div>
...
const dataArray = ref<any[]>([
  { label: 'VPC（个）', already: '123', quota: '' },
  { label: '子网（个）', already: '123', quota: '' },
  { label: '单个VPC下的路由表数目（个）', already: '123', quota: '' },
  { label: '云硬盘备份-存储库容量（GB）', already: '123', quota: '' },
  { label: '对等连接（个）', already: '123', quota: '' },
  { label: '安全组（个）', already: '123', quota: '' },
  { label: '安全组规则（个）', already: '123', quota: '' },
  { label: '弹性IP（个）', already: '123', quota: '' }
])
